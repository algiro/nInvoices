...
Every `.sh` takes `--help`. The deploy scripts read your server and domain from
`deploy.local.env` (git-ignored): copy [`deploy.local.env.example`](deploy.local.env.example)
and fill it in. Images default to `algiro/ninvoices-*` on Docker Hub; any setting can
also be passed as an environment variable (see each script's header).
