...
#   - a reverse proxy on the host terminates TLS and routes
#       $BASE/      -> ninvoices web container
#       $BASE/api/  -> ninvoices api container
#   - Keycloak is served at $KEYCLOAK_URL/realms/$KEYCLOAK_REALM
...
#
# Config: put your values in docker/deploy.local.env (git-ignored; copy deploy.local.env.example),
# or pass them as environment variables, which win over the file:
#   SSH_HOST=<required>  REMOTE_DIR=~/docker  APP_URL=https://your-domain.com/nInvoices
...
# ---- config -----------------------------------------------------------------
# Local, git-ignored values (server, domain…); variables already set in the environment win
if [[ -f deploy.local.env ]]; then
  while IFS='=' read -r key value; do
    [[ -z "$key" || "$key" == '#'* ]] && continue
    [[ -z "${!key:-}" ]] && export "$key=$value"
  done < <(tr -d '\r' < deploy.local.env)
fi
...
KEYCLOAK_URL="${KEYCLOAK_URL:-https://your-domain.com}"
API_URL="${API_URL:-/nInvoices}"
BASE="${BASE:-/nInvoices}"
APP_URL="${APP_URL:-${KEYCLOAK_URL}${BASE}}"
...
[[ -n "$SSH_HOST" ]] || die "SSH_HOST is not set: copy deploy.local.env.example to deploy.local.env and fill it in."
