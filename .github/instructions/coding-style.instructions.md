...
## Create new Instances with new keyword:    
    ```csharp
    // Good: Using Factory to hide the implementation details using the following pattern
    public interface IFPOrder
    {
        string OrderCode { get; }
        string Status { get; }
    }    

    public static class FPOrder 
    {
        public static IFPOrder Create(string orderCode, string status)
        {
            return new FPOrderImpl(orderCode, status);
        }
        // the implementation is hidden from the user
        // and the user can only use the interface
        private sealed class FPOrderImpl(string orderCode, string status) : IFPOrder
        {
